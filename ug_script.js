...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listeners
    document.getElementById('predict-btn').addEventListener('click', predictUGColleges);
...
async function loadUGData() {
    ...
    try {
        ...
        if (data.success) {
            showToast('UG data loaded successfully', 'success');
...
// Predict UG colleges
async function predictUGColleges() {
    // Get form values
    const rank = parseInt(document.getElementById('rank').value);
    ...
    // Validate inputs
    if (!rank) {
        showToast('Please enter your EAMCET rank', 'error');
    ...
    try {
        ...
        if (data.success) {
            resultsData = data;
            displayResults(data);
            showToast(`Found ${data.total} matching UG colleges`, 'success');
        } else {
            showToast(data.message, 'error');
        }
    } catch (error) {
...
function displayResults(data) {
    ...
    // Update chart
    if (data.plot) {
        document.getElementById('chart-wrapper').innerHTML = `<img src="data:image/png;base64,${data.plot}" alt="UG Recommendations Chart" style="max-width: 100%; max-height: 100%; width: auto; height: auto; object-fit: contain;">`;
    }
    ...
    // Update table info
    document.getElementById('table-info').innerHTML = `Showing ${total} UG colleges • Sorted by admission chance`;
}
...
async function downloadCSV() {
    ...
    try {
        ...
        if (data.success) {
            ...
            a.href = url;
            a.download = `EAMCET_Predictions_Rank${resultsData.rank}_${resultsData.category}_${resultsData.gender}.csv`;
